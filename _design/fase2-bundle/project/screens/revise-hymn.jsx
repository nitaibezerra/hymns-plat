...
No meu caminho`);
  ...
  const [editorView, setEditorView] = useStateR("write"); // write | ocr | diff
      ...
      <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0, minHeight: 1100}}>
        ...
        <div style={{padding: "32px 40px", borderRight: "1px solid var(--rule)", background: "var(--paper)"}}>
          <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16}}>
            <div className="eyebrow">EDITOR · TEXTO</div>
            {/* Subtle alt-view toggle */}
            <div style={{display: "flex", gap: 4, padding: 3, background: "var(--paper-soft)", border: "1px solid var(--rule)", borderRadius: 999}}>
              {[
                {k: "write", l: "Escrever"},
                {k: "ocr", l: "OCR cru"},
                {k: "diff", l: "Diff vs OCR"},
              ].map(v => (
                <button key={v.k} onClick={() => setEditorView(v.k)} style={{
                  padding: "4px 11px", border: 0, borderRadius: 999, cursor: "pointer",
                  background: editorView === v.k ? "var(--ink)" : "transparent",
                  color: editorView === v.k ? "var(--paper)" : "var(--ink-soft)",
          ...
          {editorView === "ocr" && (
            <pre style={{
              ...
              color: "var(--ink-mute)", minHeight: 280,
            }}>{`Sol da manhã
Que iluminna
O meu coraçao
Na hora da oraçao
...
No meu camninho`}</pre>
          ...
          {editorView === "diff" && (
            <div style={{
              fontFamily: "var(--font-mono)", fontSize: 13, lineHeight: 1.7,
              background: "var(--paper-soft)", border: "1px solid var(--rule)",
              borderRadius: 8, padding: 24, whiteSpace: "pre-wrap", minHeight: 280,
            }}>
              <div style={{color: "var(--ink-mute)"}}>Sol da manhã{"\n"}</div>
              <DiffLine before="Que iluminna" after="Que ilumina"/>
