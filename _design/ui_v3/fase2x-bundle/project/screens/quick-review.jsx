...
const QuickReviewScreen = () => {
  ...
  const styleInputRef = useRefQ(null);
  ...
  // Keyboard shortcuts
  // B6: ignorar quando foco está em QUALQUER input/textarea/select/contenteditable,
  // não só repsInputRef (era bug em produção).
  useEffectQ(() => {
    const onKey = (e) => {
        ...
        <div style={{padding: "40px 48px", background: "var(--paper)", display: "flex", flexDirection: "column"}}>
          ...
          <div style={{marginBottom: 36}}>
            ...
            <div style={{display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8}}>
              {STYLE_OPTS.map(o => (
                <StyleTile key={o.key}
                  ...
                  shortcut={o.key}
                  label={o.label}
                  onClick={() => { setStyle(o.label); styleInputRef.current && styleInputRef.current.focus({preventScroll: true}); styleInputRef.current && styleInputRef.current.blur(); }}
                />
              ))}
            </div>
            {/* B5: input livre — source of truth. Presets preenchem este input.
                Estilos fora dos 3 presets ("Hino", "Mestre", etc.) são aceitos. */}
            <div style={{position: "relative", marginTop: 10}}>
              ...
              />
              {style && !STYLE_OPTS.some(o => o.label === style) && (
                <span className="mono" style={{
                  position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)",
                  fontSize: 9, letterSpacing: ".1em",
                  color: "var(--gold)",
                  padding: "2px 6px", borderRadius: 3,
                  border: "1px solid color-mix(in oklab, var(--gold) 50%, var(--rule))",
