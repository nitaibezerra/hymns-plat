...
class TestGlobalStatsEndpoint:
    ...
    def test_active_reviewers_counts_recent_revisions(self, client, user_factory, hymn_book_factory, hymn_factory):
        ...
        HymnRevision.objects.filter(pk=old.pk).update(revised_at=timezone.now() - timedelta(days=60))
        resp = client.get(reverse("hymns:api_global_stats"))
...
class TestEditorResumeEndpoint:
    ...
    def test_returns_last_unreviewed_hymn_for_user(self, authenticated_client, hymn_book_factory, hymn_factory):
...
class TestUserHeatmapEndpoint:
    def test_returns_per_day_counts_last_year(self, client, user_factory, hymn_book_factory, hymn_factory):
...
class TestHymnHistoryDrawer:
    ...
    def test_drawer_creation_event_for_ocr_hymn(self, authenticated_client, hymn_book_factory, hymn_factory):
        from django.contrib.auth.models import Group
